...
export function TimetableWorkspace({ runId }: { readonly runId: string }) {
  ...
  const [entities, setEntities] = useState<TimetableEntityPage | null>(null);
  const [entityId, setEntityId] = useState("");
  const [rowOffset, setRowOffset] = useState(0);
  const [page, setPage] = useState<SavedTimetablePage | null>(null);
  ...
  useEffect(() => {
    ...
    setLoadingRows(true);
    void loadSavedTimetable(runId, view, entityId, rowOffset, ROW_PAGE_SIZE).then((result) => {
      if (!stale) setPage(result);
    }).catch((error: unknown) => {
      if (!stale) setFailure(commandError(error));
    }).finally(() => { if (!stale) setLoadingRows(false); });
    return () => { stale = true; };
  }, [runId, view, entityId, rowOffset]);

  useEffect(() => { if (inspectedId) inspectedHeading.current?.focus(); }, [inspectedId]);

  const currentEntities = entities?.runId === runId && entities.view === view && entities.offset === entityOffset ? entities : null;
  const currentPage = page?.runId === runId && page.selection.id === entityId && page.offset === rowOffset ? page : null;
  ...
  return <section className="timetable-workspace" aria-labelledby={`${labelId}-title`}>
    <header className="timetable-heading">
      <div><span className="timetable-eyebrow">已保存运行</span><h2 id={`${labelId}-title`}>课表工作台</h2></div>
      <span className="timetable-readonly">只读查看</span>
    </header>
    <p className="timetable-intro">查看原始版本的排课结果。打开时会重新校验；这些结果尚未采用为正式课表。</p>
    ...
    {currentPage && <>
      <div className="timetable-result-meta"><strong>{currentPage.selection.label}</strong><span>来源版本 {currentPage.projectRevision}</span>
        <span>独立校验通过</span>{currentPage.selectedAttemptIndex !== null && <span>本次成班候选 {currentPage.selectedAttemptIndex + 1}</span>}</div>
      <div className="timetable-grid-scroll" tabIndex={0} role="region" aria-label={`${currentPage.selection.label}周课表，可横向滚动`}>
        <table className="timetable-grid">
          <caption>{currentPage.selection.label} · 周课表</caption>
      ...
      <details className="timetable-quality"><summary>查看整份课表的质量评分与来源</summary>
        ...
        <dl><dt>运行 ID</dt><dd><code>{currentPage.runId}</code></dd><dt>原导入摘要</dt><dd><code>{currentPage.sourcePayloadHash}</code></dd>
